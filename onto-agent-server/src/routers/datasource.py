...
async def get_datasource(datasource_id: str, db: AsyncSession = Depends(get_db)) -> dict:
...
@router.post("")
async def create_datasource(data: DatasourceCreate, db: AsyncSession = Depends(get_db)) -> dict:
...
async def delete_datasource(datasource_id: str, db: AsyncSession = Depends(get_db)) -> dict:
    result = await db.execute(select(Datasource).where(Datasource.id == datasource_id))
    datasource = result.scalar_one_or_none()
...
async def test_connection(datasource_id: str, db: AsyncSession = Depends(get_db)) -> dict:
    result = await db.execute(select(Datasource).where(Datasource.id == datasource_id))
    datasource = result.scalar_one_or_none()
